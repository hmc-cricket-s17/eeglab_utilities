%eegplugin_selectICA() - SelectICA plugin
%
% Usage:
...
function eegplugin_selectICA(fig, trystrs, catchstrs)
    %global EEG
    if nargin < 3
        error('eegplugin_amica requires 3 arguments');
    end;
    
    % add selectICA folder to path
    % -----------------------
    if ~exist('runamica')
        p = which('eegplugin_selectICA');
        p = p(1:findstr(p,'eegplugin_selectICA.m')-1);
        addpath(p);
    end;
